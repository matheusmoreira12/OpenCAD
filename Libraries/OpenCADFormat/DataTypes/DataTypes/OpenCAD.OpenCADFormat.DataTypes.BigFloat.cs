...
namespace OpenCAD.OpenCADFormat.DataTypes
{
    ...
    public class BigFloat : IComparable, IComparable<BigFloat>, IEquatable<BigFloat>
    {
        ...
        private BigInteger calculateDecimalsInt(BigInteger remainder, BigInteger denominator, int decimalDigits)
        {
            return (remainder * BigInteger.Pow(10, decimalDigits)) / denominator;
        }

        private string generateDecimalsStr(BigInteger remainder, BigInteger denominator, int decimalDigits)
        {
            BigInteger decimals = calculateDecimalsInt(remainder, denominator, decimalDigits);

            return $"{decimals}".PadLeft(decimalDigits, '0');
        }

        public string ToString(IFormatProvider provider)
        {
            NumberFormatInfo numberFormatInfo = NumberFormatInfo.GetInstance(provider); //Get number format

            string decimalSeparator = numberFormatInfo.NumberDecimalSeparator; //Retrieve decimal separator
            int decimalDigits = numberFormatInfo.NumberDecimalDigits; //Retrieve number of decimal cases

            BigFloat factored = Factor(); //Get factored number

            BigInteger remainder;
            BigInteger result = BigInteger.DivRem(factored.numerator, factored.denominator, out remainder);

            if (remainder == 0 || decimalDigits == 0)
                return result.ToString(); //No extra decimal cases are required, return here.

            var decimalsStr = generateDecimalsStr(remainder, factored.denominator, decimalDigits);

            return $"{result}{decimalSeparator}{decimalsStr}"; //Concatenate strings
        }

        public string ToMixString()
        {
            BigFloat factored = Factor();
            ...
            if (remainder == 0)
                ...
                return $"{result}, {remainder}/{factored.denominator}";
        }
        ...
        public string ToRationalString()
        {
            ...
            return factored.numerator + " / " + factored.denominator;
        }
        ...
        {
            return Parse(value, CultureInfo.CurrentCulture.NumberFormat);
        }

        public static BigFloat Parse(string value, IFormatProvider provider)
        {
        ...
        {
            return TryParse(value, CultureInfo.CurrentCulture.NumberFormat, out result);
        }

        public static bool TryParse(string value, IFormatProvider provider, out BigFloat result)
        {
            try
            {
                result = BigFloat.Parse(value, provider);
                return true;
            }
        ...
        private BigFloat Factor()
        {
            ...
            return new BigFloat(numerator / factor, denominator / factor);
        }
